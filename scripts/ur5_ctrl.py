...
def get_gazebo_model_name(model_name, vision_model_pose):
    ...
    for gazebo_model_name, model_pose in zip(models.name, models.pose):
        ...
        if ds <= epsilon:  #模型的位置与视觉模型位置的距离，如果距离在epsilon范围内，则认为找到了匹配的模型。
...
def straighten(model_pose, gazebo_model_name):
    ...
    """
    # 确定LEGO朝向相机的轴
    facing_direction = get_axis_facing_camera(model_quat)  #判断物体朝上、朝下，还是侧
    # 计算抓手接近方向的角度
    ...
    # Calculate approach quat#  计算抓手角度
    approach_quat = get_approach_quat(facing_direction, approach_angle)

    # Get above the object# 移动到物体上方
    controller.move_to(x, y, target_quat=approach_quat)   #用差值计算去实现运动，去到抓取的位置

    # Calculate target quat    #重新
    regrip_quat = DEFAULT_QUAT
    if facing_direction == (1, 0, 0) or facing_direction == (0, 1, 0):  # Side  #是偏着的位置
    ...
    """
        Grip the model
    """
    if facing_direction == (0, 0, 1) or facing_direction == (0, 0, -1):    #抓取位置
        closure = model_size[0]              # 夹取闭合程度为模型的宽度
        z = SURFACE_Z + model_size[2] / 2    # 目标位置的z坐标
    ...
    controller.move_to(z=z, target_quat=approach_quat)    # 移动到目标位置的途中位置（途中位置由approach_quat决定
    close_gripper(gazebo_model_name, closure)    #夹紧模型 ------------------抓取---------------------

    """
        Straighten model if needed    如果需要，调整模型的姿态
    """
...
def get_axis_facing_camera(quat):
    axis_x = np.array([1, 0, 0])# 定义坐标轴
    axis_y = np.array([0, 1, 0])
    axis_z = np.array([0, 0, 1])
    new_axis_x = quat.rotate(axis_x) # 定义坐标轴，这个应该就是物体的方向
    new_axis_y = quat.rotate(axis_y)
    new_axis_z = quat.rotate(axis_z)
    # get angle between new_axis and axis_z
    angle = np.arccos(np.clip(np.dot(new_axis_z, axis_z), -1.0, 1.0))    # 计算新坐标轴和Z轴之间的夹角  |  物体方向和z轴的夹角
    # get if model is facing up, down or sideways
    if angle < np.pi / 3:   # 判断模型朝向的方向：向上、向下或侧向
        return 0, 0, 1
...
if __name__ == "__main__":
    ...
    rospy.sleep(0.5)
    legos = get_legos_pos(vision=True) # 获取LEGO模型的位置信息 （要么视觉、要么Gazebo中的默认信息）
    legos.sort(reverse=True, key=lambda a: (a[1].position.x, a[1].position.y))# 按照LEGO模型的位置进行排序

    for model_name, model_pose in legos:
        open_gripper() # 打开夹爪
        try:
            model_home = MODELS_INFO[model_name]["home"]  # 获取模型的初始位置和尺寸
        ...
        try:
            gazebo_model_name = get_gazebo_model_name(model_name, model_pose)# 获取模型在Gazebo中的名称，感觉就是视觉模型对比默认的模型，看看是否位置正确
        except ValueError as e:
            print(e)
            continue

        # Straighten lego   # 调整LEGO模型的姿态
        straighten(model_pose, gazebo_model_name)            #抓取
        controller.move(dz=0.15)   #网上抬一点

        """
            Go to destination前往目标位置
        """
        x, y, z = model_home
        ...
        # Lower the object and release# 降低物体高度并释放
        controller.move_to(x, y, z)                        #前往拜访的位置
        set_model_fixed(gazebo_model_name)
        open_gripper(gazebo_model_name)                    #松开夹爪
    ...
    controller.move_to(*DEFAULT_POS, DEFAULT_QUAT)  # 将机械臂移动到默认位置
    open_gripper()
    rospy.sleep(0.4)